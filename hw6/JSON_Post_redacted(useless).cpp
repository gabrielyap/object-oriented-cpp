      ...
      for (i = 0; i < (myv["actions"]["count"]).asInt(); i++)
	{
	  ...
	  if (((l_jv["avatar_name"]).isNull() != true) &&
	      ...
	      ((l_jv["type"]).isNull() != true))
	    {
	      ...
	      Person *action_p_ptr = new Person((l_jv["avatar_name"]).asString(),
					       (l_jv["vsID"]).asString()); 
	      JvTime *action_cr_time_ptr =
		new JvTime(((l_jv["created_time"]).asString()).c_str());
	      Action * action_ptr = new Action(l_jv["type"].asString(),
						  action_p_ptr, action_cr_time_ptr);
...
...
  ...
  // "updated_time"
  my_post_ptr->is_updated = (myv["is_updated"]).asBool();
